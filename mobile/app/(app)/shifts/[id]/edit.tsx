// app/(app)/shifts/[id]/edit.tsx
// Edit an existing shift's details (title, date, times, location, capacity, event lead).
// Pre-populates from GET /api/shifts/:id; saves via PATCH /api/shifts/:id.
import { useState, useEffect } from 'react'
import {
  View, Text, TextInput, TouchableOpacity, ScrollView,
  Modal, Platform, Pressable, Alert, ActivityIndicator,
} from 'react-native'
...
import { shiftsApi, type ShiftDetail } from '../../../../lib/api'
import { toISODate, toHHMM, formatTime } from '../../../../lib/date'

// ---------------------------------------------------------------------------
// Schema (same as new.tsx)
// ---------------------------------------------------------------------------
...
export default function EditShiftScreen() {
  ...
  // Pre-populate form once shift data loads
  useEffect(() => {
    if (!shift) return
    reset({
      title:                 shift.title,
      ...
      event_lead_first_name: shift.event_lead_first_name,
      event_lead_last_name:  shift.event_lead_last_name,
      event_lead_mobile:     shift.event_lead_mobile ?? '',
    })
    setShiftDate(new Date(shift.date + 'T12:00:00'))
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      Alert.alert('Error', detail ?? 'Failed to update shift.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-4 flex-row items-center">
        ...
        <Text className="text-xl font-bold text-gray-900 flex-1">Edit Shift</Text>
      </View>
